...
export default function Projects() {
  ...
  const [active, setActive] = useState(false);
    ...
    return (
      ...
      className="flex-2 w-full max-w-[800px] justify-center animate-fade-in-down text-stone-50">
        
        <div className="group">
          <div className="flex justify-center mt-5 group-hover:[--glowing:20px] drop-shadow-glow transition-all duration-1000">
          <Line active={active}/>
            <h1 className="flex justify-center font-trip text-center font-bold text-5xl hover:text-my-blue">
                <span className={`tracking-[1rem] ${active ? "tracking-[2rem]" : ""} transition-all duration-1000`}>PROJETO</span>
...
const Line = (props: {active: boolean}) => {
  return (
    <div className={`h-[3rem] w-[1px] bg-stone-50  mx-3 transition-all opacity-50 duration-700 scale-[0%] group-hover:bg-my-blue ${props.active ? "scale-[100%]" : ""}`}/>
    )
}